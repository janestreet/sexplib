...
module type S = sig @@ portable
  ...
  (** [size sexp]
      @return
        [(n_atoms, n_chars)], where [n_atoms] is the number of atoms in S-expression
        [sexp], and [n_chars] is the number of characters in the atoms of the
        S-expression. *)
  val size : t -> int * int

  (** {6 Scan functions} *)

  (** [scan_sexp ?buf lexbuf] scans an S-expression from lex buffer [lexbuf] using the
      optional string buffer [buf] for storing intermediate strings. *)
  val scan_sexp : ?buf:Buffer.t -> Lexing.lexbuf -> t @@ nonportable
  ...
  val scan_sexps : ?buf:Buffer.t -> Lexing.lexbuf -> t list @@ nonportable

  (** [scan_rev_sexps ?buf lexbuf] same as {!scan_sexps}, but returns the reversed list
      and is slightly more efficient. *)
  val scan_rev_sexps : ?buf:Buffer.t -> Lexing.lexbuf -> t list @@ nonportable

  (** [scan_sexp_opt ?buf lexbuf] is equivalent to [scan_sexp ?buf lexbuf] except that it
      returns [None] when the eof is reached. *)
  val scan_sexp_opt : ?buf:Buffer.t -> Lexing.lexbuf -> t option @@ nonportable

  (** [scan_iter_sexps ?buf ~f lexbuf] iterates over all S-expressions scanned from lex
      buffer [lexbuf] using function [f], and the optional string buffer [buf] for storing
  ...
  module Parse_pos : sig
    ...
    (** [create ?text_line ?text_char ?buf_pos ?global_offset ()]
        @return a parse position with the given parameters.
    ...
    (** [with_buf_pos t pos]
        @return a copy of the parse position [t] where [buf_pos] is set to [pos]. *)
  ...
  module Cont_state : sig
    ...
    (** [to_string cont_state] converts state of parser continuation [cont_state] to a
        string. *)
  ...
  type ('a, 't) parse_result = ('a, 't) Pre_sexp.parse_result =
    ...
    (** [Done (t, parse_pos)] finished parsing an S-expression. Current parse position is
        [parse_pos]. *)
    | Cont of Cont_state.t * ('a, 't) parse_fun
    (** [Cont (cont_state, parse_fun)] met the end of input before completely parsing an
        S-expression. The user has to call [parse_fun] to continue parsing the
        S-expression in another buffer. [cont_state] is the current parsing state of the
        continuation. NOTE: the continuation may only be called once and will raise
        [Failure] otherwise! *)
  ...
  module (Annotated @@ nonportable) : sig
    ...
    (** Exception associated with conversion errors. First argument describes the
        location, the second the reason. *)
    ...
    (** [get_sexp annot_sexp]
        @return S-expression associated with annotated S-expression [annot_sexp]. *)
    val get_sexp : t -> Type.t

    (** [get_range annot_sexp]
        @return the range associated with annotated S-expression [annot_sexp]. *)
    val get_range : t -> range

    (** [find_sexp annot_sexp sexp]
        @return
          [Some res] where [res] is the annotated S-expression that is physically
          equivalent to [sexp] in [annot_sexp], or [None] if there is no such
          S-expression. *)
    val find_sexp : t -> Type.t -> t option

    (** {6 Annotated (partial) parsing} *)

    (** [parse ?parse_pos ?len str] same as {!parse}, but returns an S-expression
        annotated with location information. *)
    val parse : ?parse_pos:Parse_pos.t -> ?len:int -> string -> (string, t) parse_result

    (** [parse_bigstring ?parse_pos ?len str] same as {!parse_bigstring}, but returns an
        S-expression annotated with location information. *)
    val parse_bigstring
    ...
    (** [input_sexp ?parse_pos ic] like {!input_sexp}, but returns an annotated
        S-expression instead. *)
    val input_sexp : ?parse_pos:Parse_pos.t -> in_channel -> t

    (** [input_sexps ?parse_pos ?buf ic] like {!input_sexps}, but returns a list of
        annotated S-expressions. *)
    val input_sexps : ?parse_pos:Parse_pos.t -> ?buf:bytes -> in_channel -> t list

    (** [input_sexps ?parse_pos ?buf ic] like {!input_rev_sexps}, but returns a list of
        annotated S-expressions. *)
    val input_rev_sexps : ?parse_pos:Parse_pos.t -> ?buf:bytes -> in_channel -> t list

    (** {6 Loading of annotated S-expressions} *)

    (** NOTE: these functions should only be used if an annotated S-expression is
        required. *)

    (** [load_sexp ?strict ?buf file] like {!load_sexp}, but returns an annotated
        S-expression. *)
    val load_sexp : ?strict:bool -> ?buf:bytes -> string -> t

    (** [load_sexps ?buf file] like {!load_sexps}, but returns a list of annotated
        S-expressions. *)
    val load_sexps : ?buf:bytes -> string -> t list

    (** [load_rev_sexps ?buf file] like {!load_rev_sexps}, but returns a list of annotated
        S-expressions. *)
    ...
    (** [conv f annot_sexp] converts the S-expression associated with annotated
        S-expression [annot_sexp] using [f].
        @return
          [`Result res] on success, or [`Error (exn, sub_annot_sexp)] otherwise, where
          [exn] is the exception associated with the conversion error, and
          [sub_annot_sexp] is the annotated S-expression on which conversion failed. *)
    val conv : (Type.t -> 'a) -> t -> 'a conv

    (** [get_conv_exn ~file ~exc annot_sexp]
        @return
          the exception that would be raised for a given [file] and exception [exc] if
          conversion had failed on annotated S-expression [annot_sexp]. The format of the
          exception message is "file:line:col" *)
  ...
  (** [parse ?parse_pos ?len str] (partially) parses an S-expression in string buffer
      ...
      most [len] characters. To parse a single atom that is not delimited by whitespace it
      is necessary to call this function a second time with the returned continuation, and
      a dummy buffer that contains whitespace.

      [parse] starts parsing [str] at position [parse_pos.buf_pos]. Each subsequent
      [parse_fun] from a [Cont] uses the [buf] and [pos] that is supplied to it. The final
      [parse_fun] that returns [Done] mutates the [buf_pos] in the originally supplied
      [parse_pos], and then returns it.
  ...
  (** [input_sexp ?parse_pos ic] parses an S-expression from input channel [ic] using
      ...
      files as {!Sexp.load_sexp}, and is also slightly slower than the scan-functions. But
      it is guaranteed that [input_sexp] is only going to read data parseable as an
      S-expression. Thus, subsequent input functions will see the data immediately
      following it.
  ...
  (** [load_sexp ?strict ?buf file] reads one S-expression from [file] using buffer [buf]
  ...
  (** [load_sexp_conv ?strict ?buf file f] like {!Sexp.load_sexp}, but performs a
      ...
      {!Sexp.load_sexp} and performing conversion when there are no errors. In contrast to
      the plain S-expression loader, this function not only performs the conversion, it
      will give exact error ranges for conversion errors.
  ...
  (** [load_sexps_conv ?buf file f] like {!Sexp.load_sexps}, but performs a conversion on
      ...
      performing conversion when there are no errors. In contrast to the plain
      S-expression loader, this function not only performs the conversion, it will give
      exact error ranges for conversion errors.
  ...
  (** NOTE: for performance reasons these output functions may need to allocate large
      strings to write out huge S-expressions. This may cause problems on 32-bit
      platforms. If you think that you may need to write huge S-expressions on such
      platforms, you might want to use the pretty-printers that write to formatters
      instead (see further below). *)

  (** [output_hum oc sexp] outputs S-expression [sexp] to output channel [oc] in human
      readable form. *)
  val output_hum : out_channel -> t -> unit

  (** [output_hum_indent indent oc sexp] outputs S-expression [sexp] to output channel
      [oc] in human readable form using indentation level [indent]. *)
  val output_hum_indent : int -> out_channel -> t -> unit

  (** [output_mach oc sexp] outputs S-expression [sexp] to output channel [oc] in machine
  ...
  (** All save-functions write to a temporary file before moving it into place to avoid
      intermittent garbling of existing files, which may cause problems for other
      processes that try to read. *)

  (** [save_hum ?perm file sexp] outputs S-expression [sexp] to [file] in human readable
      form.

      @param perm default = umask *)
  val save_hum : ?perm:int -> string -> t -> unit

  (** [save_mach ?perm file sexp] outputs S-expression [sexp] to [file] in machine
      readable (i.e. most compact) form.
  ...
  (** [save_sexps_hum ?perm file sexps] outputs S-expression list [sexps] to [file] in
      human readable form, each sexp being followed by a newline.

      @param perm default = umask *)
  val save_sexps_hum : ?perm:int -> string -> t list -> unit

  (** [save_sexps_mach ?perm file sexps] outputs S-expression list [sexps] to [file] in
      machine readable form, each sexp being followed by a newline.
  ...
  (** [of_bigstring_conv bstr conv] like {!of_bigstring}, but performs type conversion
      with [conv].
      @return conversion result. *)
  val of_bigstring_conv : bigstring -> (t -> 'a) -> 'a Annotated.conv @@ nonportable
  ...
  (** Type of successful search results. [`Found] means that an S-expression was found at
      the immediate position, and [`Pos (pos, found)] indicates that it was found at
      position [pos] within a structure (= S-expression list) where [found] describes
      recursively where it was found in that structure. *)
  type found =
    [ `Found
    | `Pos of int * found
    ]

  (** Type of search results. [`Not_found] means that an S-expression was not found within
  ...
  (** [search_physical sexp ~contained]
      @return
        the search result indicating whether, and if, where the S-expression [contained]
        was found within S-expression [sexp]. *)
  val search_physical : t -> contained:t -> search_result

  (** [subst_found sexp ~subst found]
      @return
        the S-expression that results from substituting [subst] within S-expression [sexp]
